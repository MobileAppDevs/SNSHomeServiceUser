...
import 'components/bidder_item_component.dart';
import '../../../utils/colors.dart';
...
class _MyPostDetailScreenState extends State<MyPostDetailScreen> {
  ...
  Widget titleWidget({required String title, required String detail, bool isReadMore = false, required TextStyle detailTextStyle}) {
    return Column(
      ...
      children: [
        Text(title.validate(), style: secondaryTextStyle(color: black1A1C1E)),
        4.height,
        if (isReadMore)
          ReadMoreText(
            detail,
          ...
          Text(detail.validate(), style: boldTextStyle(size: 12,color: black1A1C1E)),
        20.height,
      ],
    );
  }
        ...
        children: [
          if (data.title.validate().isNotEmpty)
            titleWidget(
              ...
              detailTextStyle: boldTextAutoStyle(context: context,commonColor: black1A1C1E),
            ),
          if (data.description.validate().isNotEmpty)
            titleWidget(
              title: language.postJobDescription,
              detail: data.description.validate(),
              detailTextStyle: primaryTextStyle(color: black1A1C1E),
              isReadMore: true,
            ),
  ...
  Widget postJobServiceWidget({required List<ServiceData> serviceList}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(language.services, style: boldTextStyle(size: LABEL_TEXT_SIZE,color: Colors.black)).paddingOnly(left: 16, right: 16),
        8.height,
        AnimatedListView(
            ...
            return Container(
              ...
              child: Row(
                children: [
                  ...
                  16.width,
                  Text(data.name.validate(), style: primaryTextStyle(color: Colors.black), maxLines: 2, overflow: TextOverflow.ellipsis).expand(),
                ],
              ),
            );
          },
  ...
  Widget providerWidget(List<BidderData> bidderList, num? providerId) {
    try {
      ...
      return Column(
        ...
        children: [
          16.height,
          Text(language.assignedProvider, style: boldTextStyle(size: LABEL_TEXT_SIZE,color: Colors.black)),
          16.height,
          InkWell(
            onTap: () {
                        ...
                        children: [
                          ...
                          if (user.email.validate().isNotEmpty)
                            Marquee(
                              ...
                              child: Text(
                                user.email.validate(),
                                style: primaryTextStyle(size: 12,color: black1A1C1E),
                                maxLines: 1,
                                overflow: TextOverflow.ellipsis,
                              ),
                            ),
