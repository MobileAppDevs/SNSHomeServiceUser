...
import 'package:home_service_user/component/add_review_dialog.dart';
...
import 'package:home_service_user/model/booking_detail_model.dart';
import 'package:home_service_user/model/extra_charges_model.dart';
import 'package:home_service_user/model/package_data_model.dart';
import 'package:home_service_user/model/service_data_model.dart';
...
import 'package:home_service_user/screens/booking/booking_history_component.dart';
import 'package:home_service_user/screens/booking/component/booking_detail_handyman_widget.dart';
import 'package:home_service_user/screens/booking/component/booking_detail_provider_widget.dart';
import 'package:home_service_user/screens/booking/component/booking_status_filter_bottom_sheet.dart';
import 'package:home_service_user/screens/booking/component/countdown_component.dart';
import 'package:home_service_user/screens/booking/component/invoice_request_dialog_component.dart';
import 'package:home_service_user/screens/booking/component/price_common_widget.dart';
import 'package:home_service_user/screens/booking/component/reason_dialog.dart';
import 'package:home_service_user/screens/booking/component/service_proof_list_widget.dart';
import 'package:home_service_user/screens/booking/handyman_info_screen.dart';
import 'package:home_service_user/screens/booking/provider_info_screen.dart';
import 'package:home_service_user/screens/booking/shimmer/booking_detail_shimmer.dart';
import 'package:home_service_user/screens/payment/payment_screen.dart';
import 'package:home_service_user/screens/review/components/review_widget.dart';
import 'package:home_service_user/screens/review/rating_view_all_screen.dart';
import 'package:home_service_user/screens/service/service_detail_screen.dart';
import 'package:home_service_user/utils/booking_calculations_logic.dart';
...
import 'component/cancel_booking_bottom_sheet.dart';
...
class _BookingDetailScreenState extends State<BookingDetailScreen> {
 ...
 /* List<ExtraChargesModel> calculateExtraCharges(
      {required BookingDetailResponse res})
  {
    List<ExtraChargesModel> list = [];
    List<TaxData> taxes = res.taxes.validate();
    if (taxes.isEmpty && res.bookingDetail != null) {
      taxes = res.bookingDetail!.taxes.validate();
    }
    if (taxes.isEmpty) return list;

    num quantity = res.bookingDetail!.quantity.validate(value: 1);
    num servicePrice = res.service != null ? res.service!.price.validate() : 0;

    if (res.bookingDetail!.isPackageBooking &&
        res.bookingDetail!.bookingPackage != null) {
      servicePrice = res.bookingDetail!.bookingPackage!.price.validate();
    }

    num discountPercent = res.bookingDetail!.discount.validate();
    num discountedPricePerUnit =
        servicePrice - (servicePrice * discountPercent / 100);

    num currentBasePrice = discountedPricePerUnit;

    taxes.forEach((element) {
      num taxAmount = 0;
      if (element.type == TAX_TYPE_PERCENT) {
        taxAmount = currentBasePrice * element.value.validate() / 100;
      } else {
        taxAmount = element.value.validate();
      }

      list.add(ExtraChargesModel(
        bookingId: res.bookingDetail!.id,
        title: element.title,
        price: taxAmount,
        qty: quantity,
        *//*serviceId: res.bookingDetail!.serviceId,*//*
      ));

      currentBasePrice += taxAmount;
    });
  ...
  Widget _pendingMessage({required BookingDetailResponse snap}) {
    if (snap.bookingDetail!.status == BookingStatusKeys.pending)
      return Container(
        padding: EdgeInsets.all(16),
        color: pending.withOpacity(0.07),
        width: context.width(),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            if (snap.bookingDetail!.status ==
              ...
              Text(language.advancePaymentMessage,
                  style: primaryTextStyle(color: pending))
            else
        ...
        Container(
          ...
          child: ListView.separated(
            ...
            separatorBuilder: (BuildContext context, int index) {
              return Divider(height: 0, color: greyBFBFBF);
            },
          ),
        ),
      ],
      ...
      return Column(
        ...
        children: [
          ...
          Text(language.lblAboutHandyman,
              style: boldTextStyle(size: LABEL_TEXT_SIZE, color: pureBlack)),
          16.height,
          Column(
                  ...
                  children: [
                    ...
                    Text("#" + bookingData.paymentId.toString(),
                        style: boldTextAutoStyle(
                            context: context, commonColor: black1A1C1E)),
                  ],
                ),
                4.height,
                ...
                Row(
                  children: [
                    ...
                    Text(language.yourReview,
                            style: boldTextStyle(
                                size: LABEL_TEXT_SIZE, color: pureBlack))
                        .expand(),
                    ic_edit_square_png
                        .iconImage(size: 16)
                        .paddingAll(8)
                    ...
                    ic_delete_png.iconImage(size: 16).paddingAll(8).onTap(() {
                      showDialog(
                        context: context,
  ...
  Widget packageWidget({required BookingPackage? package}) {
    ...
    return Column(
      ...
      children: [
        16.height,
        Text(language.includedInThisPackage,
            style:
                boldTextAutoStyle(context: context, commonColor: black1A1C1E)),
        ListView.builder(
          shrinkWrap: true,
          physics: NeverScrollableScrollPhysics(),
      ...
      return CommonButton(
        ...
        onTap: () async {
          if (bookingResponse.bookingDetail!.status ==
              ...
              bookingResponse.bookingDetail!.status == BookingStatusKeys.hold) {
    ...
    CustomDialogUtils.showConfirmDialogCustom(
      ...
      onAccept: (c) async {
        ...
        if (status.bookingDetail!.isHourlyService ) {
          BookingAmountModel bookingAmountModel = await finalCalculations(
            ...
            extraCharges: status.bookingDetail!.extraCharges
               /* calculateExtraCharges(res: await future!)*/,
                ...
                Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      8.height,
                      bookingIdWidget(),
                      Divider(height: 13, color: greyBFBFBF),
                      10.height,
                      ...
                      16.height,
                      Divider(height: 0, color: greyBFBFBF),
