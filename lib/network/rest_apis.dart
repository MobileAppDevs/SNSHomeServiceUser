...
import 'package:home_service_user/model/base_response_model.dart';
...
import 'package:home_service_user/model/booking_list_model.dart';
import 'package:home_service_user/model/booking_status_model.dart';
import 'package:home_service_user/model/category_model.dart';
import 'package:home_service_user/model/city_list_model.dart';
import 'package:home_service_user/model/country_list_model.dart';
import 'package:home_service_user/model/dashboard_model.dart';
import 'package:home_service_user/model/get_my_post_job_list_response.dart';
import 'package:home_service_user/model/login_model.dart';
import 'package:home_service_user/model/notification_model.dart';
import 'package:home_service_user/model/post_job_detail_response.dart';
import 'package:home_service_user/model/provider_info_response.dart';
import 'package:home_service_user/model/provider_list_model.dart';
...
import 'package:home_service_user/model/service_response.dart';
import 'package:home_service_user/model/service_review_response.dart';
import 'package:home_service_user/model/state_list_model.dart';
import 'package:home_service_user/model/user_data_model.dart';
import 'package:home_service_user/model/user_wallet_history.dart';
import 'package:home_service_user/model/verify_transaction_response.dart';
import 'package:home_service_user/network/network_utils.dart';
...
Future<LoginResponse> loginUser(Map request, {bool isSocialLogin = false}) async {
  try {
    ...
    if (res.userData != null) {
     /* if (*//*res.userData!.userType != USER_TYPE_USER*//* res.userData!.userType==USER_TYPE_PROVIDER||res.userData!.userType==USER_TYPE_HANDYMAN) {
    ...
    builder: (p0) {
      return Column(
        ...
        children: [
          ...
          32.height,
          Text(language.lblLogoutTitle, style: boldTextStyle(size: 18,fontFamily: GoogleFonts.mulish().fontFamily,color: black1A1C1E)),
          16.height,
          ...
          Row(
            children: [
              AppButton(
                child: Text(language.lblNo, style: boldTextAutoStyle(context: context,commonColor: black1A1C1E)),
                color: greyAEAEB2,
                elevation: 0,
                onTap: () {
                  finish(context);
...
Future<ValidateOtpResponse> validateOtp(Map request) async {
  return ValidateOtpResponse.fromJson(await handleResponse(await buildHttpResponse('validate-otp', request: request, method: HttpMethodType.POST)));
}

Future<BaseResponseModel> checkUniqueUsername(Map request) async {
  return BaseResponseModel.fromJson(await handleResponse(await buildHttpResponse('check-unique-username', request: request, method: HttpMethodType.POST)));
}
...
Future<List<PaymentSetting>> getPaymentGateways({bool requireCOD = true, bool requireWallet = true}) async {
  try {
    ...
    if (requireWallet && appConfigurationStore.isEnableUserWallet) { // added manually wallet to list remove wallet after discussion
...
Future<BaseResponseModel> addAddress(Map request) async {
  return BaseResponseModel.fromJson(await handleResponse(await buildHttpResponse('user-addresses', request: request, method: HttpMethodType.POST)));
}

Future<List<AddressModel>> getaddressList() async {
  final response = await handleResponse(await buildHttpResponse('user-addresses', method: HttpMethodType.GET));
  if (response is List) {
