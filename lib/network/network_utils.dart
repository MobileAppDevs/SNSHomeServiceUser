...
Future handleResponse(Response response, {HttpResponseType httpResponseType = HttpResponseType.JSON}) async {
  ...
  if (response.statusCode == 400) {
    ...
    throw responseBody['message'] ?? 'Forbidden';
  } else if (response.statusCode == 404) {
    throw '${language.pageNotFound}';
  }else if (response.statusCode == 401){ // this  added for  forget password api
    final responseBody = jsonDecode(response.body);
    throw responseBody['message'] ?? 'Something went wrong';
  } else if(response.statusCode == 422){ // this is added for userName Api
    final responseBody = jsonDecode(response.body);
    String message = responseBody['message'] ?? 'Something went wrong';
    if (responseBody['errors'] != null) {
      Map errors = responseBody['errors'];
      String errorDetails = errors.entries
          .map((e) => (e.value as List).join(', '))
          .join(', ');
      throw '$message, $errorDetails';
    } else {
      throw message;
    }
