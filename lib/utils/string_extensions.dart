...
extension strEtx on String {
  Widget iconImage({double? size, Color? color, BoxFit? fit}) {
    return Image.asset(
      ...
      color: color ?? (appStore.isDarkMode ? appTextSecondaryColor/*Colors.white*/ : appTextSecondaryColor),
      errorBuilder: (context, error, stackTrace) {
        return Image.asset(ic_no_photo, height: size ?? 24, width: size ?? 24);
      },
    );
  }
  Widget iconImageSimple({double? size, Color? color, BoxFit? fit}) {
    return Image.asset(
      this,
      height: size ?? 24,
      width: size ?? 24,
  ...
  Widget iconImageVariable({double? height,double? width, Color? color, BoxFit? fit}) {
    return Image.asset(
      this,
      height: height?? 24,
      width: width ?? 24,
      fit: fit ?? BoxFit.cover,
      color: color ?? (appStore.isDarkMode ? Colors.white : Colors.white),
      errorBuilder: (context, error, stackTrace) {
        return Image.asset(ic_no_photo, height: height ?? 24, width: width ?? 24);
      },
    );
  }

  Widget iconSvgImage({double? size, Color? color, BoxFit? fit}) {
