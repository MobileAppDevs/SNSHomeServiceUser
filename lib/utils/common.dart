...
import 'package:home_service_user/utils/permissions.dart';
...
import '../component/html_widget.dart';
import '../component/location_service_dialog.dart';
import '../component/new_update_dialog.dart';
import '../main.dart';
...
ThemeMode get appThemeMode => appStore.isDarkMode ? ThemeMode.light/*ThemeMode.dark*/ : ThemeMode.light;
...
InputDecoration inputDecoration(BuildContext context, {Widget? prefixIcon, String? labelText, double? borderRadius,String? hint,TextStyle? hintStyle}) {
  return InputDecoration(
    ...
    enabledBorder: OutlineInputBorder(// Disable state border
      borderRadius: radius(borderRadius ?? defaultRadius),
      borderSide: BorderSide(color: greyE5E8E9, width: 1.0),
    ),
...
void showPasswordRequirementDialog(BuildContext context) {
  showDialog(
    context: context,
    builder: (BuildContext context) {
      return AlertDialog(
        title: Text("Password Requirement", style: boldTextStyle(weight: FontWeight.w700,color:primaryColor,size: 13 )),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text("- Password length must be greater than 6.", style: secondaryTextStyle()),
            4.height,
            Text("- Must contain at least one alphanumeric character.", style: secondaryTextStyle()), // Logic checks for at least one letter AND one number separately, generally confusingly called alphanumeric but I'll stick to user request "alpha numeric" implies letters and numbers.
            4.height,
            Text("- Must contain at least one special character.", style: secondaryTextStyle()),
          ],
        ),
        actions: [
          TextButton(
            ...
            child: Text(language.lblOk, style: primaryTextStyle(color: primaryColor)),
          ),
        ],
      );
    },
...
TextStyle boldTextAutoStyle({
  ...
  Color finalColor = commonColor ??
      (isDark ? (darkColor ?? Colors.white) : (lightColor ?? Colors.black));
