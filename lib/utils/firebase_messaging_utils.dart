...
Future<void> subscribeToFirebaseTopic() async {
  if (!appStore.isLoggedIn) {
    ...
    // comment code for fetching  getAPNSToken
    if (Platform.isIOS) {
    ...
    if (fcmToken != null) {
      appConfigurationStore.setFcmToken(fcmToken);
      print('FCM Token inside subscribeFirebase1: $fcmToken');
      log('FCM Token inside subscribeFirebase: $fcmToken');
      await FirebaseMessaging.instance.subscribeToTopic('user_${appStore.userId}');
      await FirebaseMessaging.instance.subscribeToTopic(USER_APP_TAG);
    }
